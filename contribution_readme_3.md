...
**Issue:** https://github.com/Raftersecurity/rafter-cli/pull/190  
**Status:** Merged

---

## Why I Chose This Issue
I chose this issue because it gave me the chance to work on a real-world project that connects both Node (TypeScript) and Python. The task was clear and meaningful: adding secret scanning for MailChimp API Tokens with precise regex patterns to avoid false positives.
...
The secret scanner was missing a dedicated validation pattern to detect MailChimp API Tokens (PATs). Without this, users could accidentally commit active MailChimp credentials to their repositories without triggering a warning from the tool.

### Expected Behavior

The tool should automatically scan files and detect MailChimp PATs across both supported ecosystems (Node and Python) using an identical, precise regex pattern. It should identify tokens starting with the distinct prefix and flag them cleanly without causing high false-positive rates.

### Current Behavior

Prior to this fix, strings matching the MailChimp token format were completely ignored during scans, as no pattern or corresponding test definitions existed in either codebase.
...
1. Feed a sample payload containing a structurally valid mock MailChimp token (e.g., dop_v1_ followed by 64 hexadecimal characters) into the scanner engine.
...
- **Commit showing reproduction:** https://github.com/Raftersecurity/rafter-cli/pull/190/changes/eab6703cceb76a64b68ca7c7bd808e86a1e62d7f
...
The root cause was a coverage gap in the scanner's pattern definition arrays. The regex lists for both the Node and Python engines lacked the explicit cryptographic signatures, structural formats, and prefix rules necessary to track down exposed MailChimp API Tokens. Because these dictionaries were unpopulated, the scanner string-matching loops passed over valid tokens without triggering alerts.
...
**Understand:** The application needs to recognize and catch exposed MailChimp API Tokens (dop_v1_) in codebase files across both the Node and Python execution environments, using identical regex behavior and non-live test assertions.
...
1. Modify node/src/scanners/secret-patterns.ts to add the MailChimp token regex pattern object with a severity rating of critical.
...
**Implement:** https://github.com/Raftersecurity/rafter-cli/pull/190/changes/eab6703cceb76a64b68ca7c7bd808e86a1e62d7f
...
- What was built: Implemented a new regex pattern block to detect MailChimp API Tokens (dop_v1_) across both the Node (TypeScript) and Python core scanning arrays, backed by custom unit tests verifying synthetic token detection in both language suites.
...
- **Key commits:** 
feat: include node implementation and tests for MailChimp pattern
...
**PR Link:** https://github.com/Raftersecurity/rafter-cli/pull/190  

**PR Description:** 
### Summary
This PR adds support for detecting MailChimp API Tokens (`dop_v1_`) across both the Node and Python regex scanners.
...
**Maintainer Feedback:**
Resolved — the Mailchimp API key pattern landed in #190 (thanks @Minh-Nguyen-2k7!) with follow-up negative-case tests in #194. Both merged to main (Node + Python, in lockstep).
**Status:** Merged
...
- MailChimp API Token Documentation: Referenced the official MailChimp platform guidelines regarding the exact structural format, prefix constraints (dop_v1_), and length configurations for Personal Access Tokens.
